...
export default function JournalPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="font-serif text-3xl text-text">Learning Journal</h1>
      <p className="mt-2 text-text-3">Your session history, auto-generated summaries, and reflections on what clicked.</p>
      ...
      {!loading && sessions.length === 0 && (
        <div className="mt-8 flex h-64 items-center justify-center rounded-lg border-2 border-dashed border-border bg-surface">
          <p className="text-xl text-text-3">No sessions yet</p>
        </div>
      )}
      ...
      {!loading && sessions.length > 0 && (
        <div className="mt-8 space-y-10">
          {dates.map((date) => (
            <div key={date}>
              <h2 className="mb-4 font-serif text-lg text-text-3">{formatDate(groups[date][0].startedAt)}</h2>
              <div className="space-y-4">
                {groups[date].map((session) => (
                  <div key={session.id} className="rounded-lg border border-border bg-white p-5 shadow-sm">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      <h3 className="font-serif text-lg text-text">{session.topicTitle ?? "Unknown Topic"}</h3>
                      {session.subject && (
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${SUBJECT_COLORS[session.subject] ?? "bg-gray-100 text-gray-700"}`}>
                          {session.subject}
                        </span>
                      )}
                      <span className="rounded-full border border-accent-border bg-accent-surface px-2 py-0.5 text-xs text-accent">
                        {session.mode}
                      </span>
                    ...
                    {session.journalSummary ? (
                      <div className="lined-paper rounded-md border border-border p-4">
                        <p className="font-study text-lg text-text-2">{session.journalSummary}</p>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleGenerateSummary(session.id)}
                        disabled={generatingSummary === session.id}
                        className="rounded-md border border-dashed border-accent-border bg-accent-surface px-4 py-2 text-sm text-accent hover:bg-accent-surface/80 disabled:opacity-50"
                      >
